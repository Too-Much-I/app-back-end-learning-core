...
public class GradingDispatchService {
    ...
    public void dispatchQuestion(QuestionGradingJob job) {
        byte[] audioBytes = restTemplate.getForObject(
                URI.create(generatePresignedGetUrl(job.getFileKey(), Duration.ofMinutes(5))),
                byte[].class
        );
        if (audioBytes == null) {
        ...
        ByteArrayResource audioResource = new ByteArrayResource(audioBytes) {
            ...
            public String getFilename() {
                return "q_%d_r%d.webm".formatted(job.getQuestionNumber(), job.getRetryCount());
            }
        };

        MultiValueMap<String, Object> body = new LinkedMultiValueMap<>();
        body.add("user_id", job.getExamId());
        body.add("mock_exam_id", GradingKeys.MOCK_EXAM_ID);
        body.add("part_number", partNumber(job.getQuestionNumber()));
        body.add("question_number", job.getQuestionNumber());
        body.add("retry_count", job.getRetryCount());
        body.add("audio_file", audioResource);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.MULTIPART_FORM_DATA);
        headers.set(IDEMPOTENCY_KEY_HEADER, job.getJobId());
    ...
    public void dispatchSummary(String examId) {
        ...
        headers.set(IDEMPOTENCY_KEY_HEADER, GradingKeys.summaryJobId(examId));
